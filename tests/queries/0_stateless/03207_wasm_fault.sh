...
CREATE OR REPLACE FUNCTION huge_allocate LANGUAGE WASM ABI PLAIN FROM 'faulty' ARGUMENTS (UInt32) RETURNS UInt32 SETTINGS max_memory = 655360;
SELECT huge_allocate(1 :: UInt32) <= 10;
SELECT huge_allocate(10 :: UInt32) == 0;

CREATE OR REPLACE FUNCTION huge_allocate LANGUAGE WASM ABI PLAIN FROM 'faulty' ARGUMENTS (UInt32) RETURNS UInt32 SETTINGS max_memory = 6553600;
SELECT 10 < huge_allocate(1 :: UInt32) AND huge_allocate(1 :: UInt32) <= 100;

CREATE OR REPLACE FUNCTION infinite_loop LANGUAGE WASM ABI PLAIN FROM 'faulty' ARGUMENTS (UInt32) RETURNS UInt32;
SELECT infinite_loop(1 :: UInt32); -- { serverError WASM_ERROR }

CREATE OR REPLACE FUNCTION fib_wasm LANGUAGE WASM ABI PLAIN FROM 'faulty' :: 'fib' ARGUMENTS (UInt32) RETURNS UInt32;
...
CREATE OR REPLACE FUNCTION write_out_of_bounds LANGUAGE WASM ABI PLAIN FROM 'faulty' ARGUMENTS (UInt32) RETURNS UInt32;
SELECT write_out_of_bounds(number:: UInt32) FROM numbers(10); -- { serverError WASM_ERROR }
DROP FUNCTION IF EXISTS write_out_of_bounds;

CREATE OR REPLACE FUNCTION read_out_of_bounds LANGUAGE WASM ABI PLAIN FROM 'faulty' ARGUMENTS (UInt32) RETURNS UInt32;
