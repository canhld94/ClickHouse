...
CREATE OR REPLACE FUNCTION returns_out_of_bounds LANGUAGE WASM ABI UNSTABLE_V0_1 FROM 'abi_buf_out_of_bounds' ARGUMENTS (UInt32) RETURNS Int32;
SELECT returns_out_of_bounds(0 :: UInt32); -- { serverError WASM_ERROR }

CREATE OR REPLACE FUNCTION returns_out_of_bounds2 LANGUAGE WASM ABI UNSTABLE_V0_1 FROM 'abi_buf_out_of_bounds' ARGUMENTS (UInt32) RETURNS Int32;
SELECT returns_out_of_bounds2(0 :: UInt32); -- { serverError WASM_ERROR }

-- module itself is ok, test properly defined function from it
CREATE OR REPLACE FUNCTION test_func LANGUAGE WASM ABI UNSTABLE_V0_1 FROM 'abi_buf_out_of_bounds' ARGUMENTS (UInt64) RETURNS UInt64 SETTINGS serialization_format = 'CSV';
